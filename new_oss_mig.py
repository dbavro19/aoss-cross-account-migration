...
class OpenSearchCrossAccountMigration:
    ...
    def _create_target_session(self) -> boto3.Session:
        ...
        try:
            ...
            # logger.info(f"Attempting to assume role: {self.target_role}")
            # logger.info(f"Host role: {sts_client.get_caller_identity()['Arn']}")
            ...
            #logger.info(f"Creds for assumed role: {credentials}")
    ...
    def refresh_target_credentials_if_needed(self) -> None:
        ...
        """
        # Skip if we're in same-account mode or using direct cross-account access
        if self.source_account_id == self.target_account_id or self.target_credentials_valid:
            return
            
        # Check if it's time to refresh credentials
        current_time = datetime.now(timezone.utc)
        if current_time - self.last_credential_refresh >= self.credential_refresh_interval:
        ...
        try:
            ...
            dest_response = await loop.run_in_executor(
                ...
                lambda: self.create_dest_ingestion_pipeline(
                    ...
                    s3_prefix=s3_prefix,
                    index_name=index,
                    iam_role=self.target_role,  # Use target role directly
