...
# Add random number/char to pipelinename/loggroup

src_host = sys.argv[1]
src_ingestion_role = sys.argv[2]
dest_host = sys.argv[3]
dest_ingestion_role = sys.argv[4]
source_region = sys.argv[5]
target_region = sys.argv[6]

credentials = boto3.Session().get_credentials()
pprint(credentials)
pipeline_client = boto3.client('osis', region_name=source_region)
logs_client = boto3.client('logs', region_name=source_region)
s3_client = boto3.client('s3')
cloudwatch_client = boto3.client('cloudwatch', region_name=source_region)
...
def create_src_ingestion_pipeline(pipeline_name, source_opensearch_endpoint, 
                             s3_bucket_name, index_name, s3_prefix, iam_role, log_group_name):
...
def create_dest_ingestion_pipeline(pipeline_name, opensearch_endpoint, 
                             s3_bucket_name, s3_prefix, index_name, iam_role, log_group_name):
...
def get_pipeline_metric_stats(pipeline_name, metric_name, start_time, end_time, period=60):
    """
    Get metric statistics from CloudWatch for an OpenSearch Ingestion Pipeline.

    Args:
        pipeline_name (str): Name of the OpenSearch Ingestion Pipeline
        metric_name (str): Name of the metric to query (e.g., 'recordsIn.count')
        start_time (datetime): Start time for the metric query
        end_time (datetime): End time for the metric query
        period (int): Period in seconds for the metric aggregation (default: 60)

    Returns:
        dict: Dictionary containing the metric statistics
    """
    print(f'start_time: {start_time} - metric: {metric_name}')
    print(f'end_time: {end_time} - metric: {metric_name}')
...
def get_migration_status(src_host, index_name, src_pipeline, metric_name, start_time, 
                             period=60, max_attempts=30, delay_seconds=60):
    src_docs_count = int(get_index_doc_count(src_host, index_name))
    
    for attempt in range(1, max_attempts + 1):
        # Update end_time to current time for each attempt
        current_end_time = datetime.now(timezone.utc)
        
        # Get current metrics count
        total_target_docs = 0
        processed_count = get_pipeline_metric_stats(
            src_pipeline, 
            metric_name, 
            start_time, 
            current_end_time, 
        ...
        total_target_docs += processed_count
        if total_target_docs == src_docs_count:
            print(f"Source documents and processed records match - {index_name}: {src_docs_count}")
            return True
            
        print(f"Attempt {attempt}/{max_attempts}: Source document count: {src_docs_count}, Processed records: {processed_count} - index: {index_name}")
        print(f'Migration is in process - {index_name}')
        time.sleep(delay_seconds)
    
    print(f"Timed out waiting for migration to complete. Source docs: {src_docs_count}, Processed: {processed_count}")
    return False
...
def cleanup_resources(pipeline_names, log_groups, delete_pipelines=True):
    ...
    if delete_pipelines:
        for pipeline_name in pipeline_names:
            try:
                x = delete_pipeline(pipeline_name)
                print(x)
            except Exception as e:
    ...
    for log_group in log_groups:
        try:
            x  = logs_client.delete_log_group(logGroupName=log_group)
            print(x)
...
async def migrate_index(index, src_host, src_ingestion_role, dest_host, dest_ingestion_role, 
                       s3_bucket_name, migration_tracker_file):
    """
    ...
    try:
        ...
        s3_prefix = f"src-{pipeline_index[:6]}/"
        src_pipeline_name = f"oss-s3-pipe-{pipeline_index[:6]}"
        ...
        pipeline_active = await loop.run_in_executor(
            None,
            lambda: wait_for_pipeline_active(src_pipeline_name, max_attempts=40, delay_seconds=60)
        )
        ...
        update_tracker(migration_tracker_file, index, checkpoint="CREATED_SOURCE_PIPELINE")
 
        
        # time.sleep(120)
        # print('Sleep for 2 minutes before checking the migration status')   
        migration_status = await loop.run_in_executor(
            None,
            lambda: get_migration_status(src_host, index, src_pipeline_name, source_metric_name, migration_start_time, 
                             period=3600, max_attempts=30, delay_seconds=60)
        )

        if not migration_status:
            update_tracker(migration_tracker_file, index, checkpoint="FAILED MIGRATION TO S3")
            raise Exception(f"Source pipeline {src_pipeline_name} - Migration Failed")

        update_tracker(migration_tracker_file, index, checkpoint="MIGRATED TO S3")

        # S3 to OSS Migration
        # 4. Create a pipeline to move data from S3 to destination opensearch serverless
        dest_pipeline_name = f"s3-oss-pipe-{pipeline_index[:6]}"
        ...
        pipeline_active = await loop.run_in_executor(
            None,
            lambda: wait_for_pipeline_active(dest_pipeline_name, max_attempts=40, delay_seconds=60)
        )
        ...
        oss_migration_status = await loop.run_in_executor(
            None,
            lambda: get_migration_status(src_host, index, dest_pipeline_name, dest_metric_name, target_migration_start_time, 
                             period=3600, max_attempts=30, delay_seconds=60)
        )

        if not oss_migration_status:
            update_tracker(migration_tracker_file, index, checkpoint="FAILED MIGRATION TO TARGET OSS")
            raise Exception(f"Destination pipeline {dest_pipeline_name} - Migration Failed")

        update_tracker(migration_tracker_file, index, checkpoint="MIGRATED TO TARGET OSS")
        ...
        if src_doc_count == dest_doc_count and src_doc_count > 0:
            print('#'*30)
            print(f"src docs and dest docs are same for index: {index}")
            print('#'*30)
...
async def migrate_opensearch_indices_async(src_host, src_ingestion_role, dest_host, dest_ingestion_role, 
                                          s3_bucket_name="auto-test-12345", max_concurrent=3):
    """
...
if __name__ == "__main__":
    ...
    # pipeline_name = "oss-s3-pipe-new-tr"

    # # Monitor the pipeline for 30 minutes, polling every 3 minutes
    # results = monitor_pipeline_metrics_live(
    #     pipeline_name=pipeline_name,
    #     duration_minutes=30,  # Total monitoring duration
    #     poll_interval_seconds=180,  # Poll every 3 minutes
    #     period=60  # 1-minute aggregation period for metrics
    # )

    # # The results contain the final cumulative metrics
    # print(f"Final results: {results}")

    # #end_time = datetime(2025,5,29, 0, 0, 0)
    # start_time = datetime(2025,5,28, 0 ,0 ,0)
    # metric_name = f"{pipeline_name}.s3.recordsIn.count"

    # end_time = datetime.now(timezone.utc).replace(tzinfo=None)
    # print(start_time)
    # print(end_time)
    # results = get_pipeline_metric_stats(pipeline_name, metric_name, start_time, end_time, period=300)
    # pprint(results)

    # # Define time range for metrics (last hour)
    ...
    # pipeline_name = "oss-s3-pipe-new-tr"
    # metrics = get_pipeline_metrics(pipeline_name, start_time, end_time)

    # # Print the results
    # print(f"Pipeline {pipeline_name} metrics:")
    # print(f"Total records in: {metrics['recordsIn']['total']}")
    # print(f"Total records out: {metrics['recordsOut']['total']}")

    # # You can also access the time series data
    # for i in range(len(metrics['recordsIn']['timestamps'])):
    #     timestamp = metrics['recordsIn']['timestamps'][i]
    #     records_in = metrics['recordsIn']['values'][i]
    #     records_out = metrics['recordsOut']['values'][i]
    #     print(f"{timestamp}: In={records_in}, Out={records_out}")  

    # # response = s3_client.list_objects_v2(Bucket='auto-test-12345', Prefix='src-git-try')
    # # print(response)
